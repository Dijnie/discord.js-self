...
This guide will show you how to set up a proxy with @discord-selfbot-sdk/bot. This may be necessary if you are deploying your bot to a server with a firewall only allowing outside traffic through the proxy.

Proxying @discord-selfbot-sdk/bot requires two components: a REST proxy and a WebSocket proxy.
...
The `@discordjs/rest` package handling HTTP requests in @discord-selfbot-sdk/bot uses the `undici` package. Accordingly, you can provide a custom `ProxyAgent` configuration to the client constructor:
