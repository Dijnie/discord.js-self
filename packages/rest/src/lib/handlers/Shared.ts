		...
		if (status >= 400 && status < 500) {
			...
			if (status === 401 && requestData.auth === true) {
				if (isDiscordError && data.code !== 0 && !authFalseWarningEmitted) {
					const errorText = `Encountered HTTP 401 with error ${data.code}: ${data.message}. Your token will be removed from this REST instance. If you are using @selfbot.js/rest directly, consider adding 'auth: false' to the request. Open an issue with your library if not.`;
