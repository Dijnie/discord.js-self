...
function resolveCanonicalReference(
	...
	if (
		...
		return {
			package: canonicalReference.packageName?.replace('@discord-selfbot-sdk/', ''),
			item: {
			...
			if (token.canonicalReference) {
				...
				return {
					...
					resolvedItem: {
						...
						packageName: resolved.package?.replace('@discord-selfbot-sdk/', ''),
			...
			if (parent?.typeParameters.some((type) => type.name === token.text)) {
				...
				return {
					...
					resolvedItem: {
						...
						packageName: resolvedParent?.package?.replace('@discord-selfbot-sdk/', ''),
					},
				};
			}
			...
			case DocNodeKind.LinkTag: {
				...
				if (codeDestination) {
					...
					return {
						...
						resolvedPackage: {
							packageName:
								resolved?.package ?? apiItem.getAssociatedPackage()?.displayName.replace('@discord-selfbot-sdk/', ''),
...
function resolveFileUrl(item: ApiDeclaredItem) {
	...
	if (fileUrl?.includes('/node_modules/')) {
		...
		// https://github.com/discordjs/discord.js/tree/main/node_modules/.pnpm/@discordjs+builders@1.9.0/node_modules/@discord-selfbot-sdk/builders/dist/index.d.ts#L1764
		// https://github.com/discordjs/discord.js/tree/main/node_modules/.pnpm/@discordjs+ws@1.1.1_bufferutil@4.0.8_utf-8-validate@6.0.4/node_modules/@discord-selfbot-sdk/ws/dist/index.d.ts#L...
